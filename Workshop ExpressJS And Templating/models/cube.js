...
class CubeModel {
    ...
    create(name, description, imageUrl, difficultyLevel) {
        return { name, description, imageUrl, difficultyLevel: +difficultyLevel }
    }
    ...
    delete(cubeId) {
        ...
        const newData = {
            lastIndex: this.data.lastIndex,
            entities: this.data.entities.filter(({ id: entityId }) => entityId !== cubeId)
        }
    ...
    getOne(id) {
        return Promise.resolve(this.data.entities.find(({ id: entityId }) => entityId === id));
    }
